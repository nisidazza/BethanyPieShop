...
namespace BethanysPieShop.Models
{
    ...
    {
        private List<Pie> _pies; // private list that is going to be initialized in the constructor if it's still null
        ...
        public IEnumerable<Pie> GetAllPies() // thi method will return the above pies list
        {
            return _pies; 
        }

        public Pie GetPieById(int pieId) // this method is going to retunr from the pies list the first pie for which the ID is equal to the passed in ID
        {
            return _pies.FirstOrDefault(pieId => pieId.Id == pieId);
        }
    }
}
